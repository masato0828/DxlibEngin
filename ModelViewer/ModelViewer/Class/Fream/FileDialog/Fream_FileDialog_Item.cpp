...
void Fream_FileDialog_Item::Update(FileData*& nowselect,std::filesystem::path& fileFullPath, std::wstring& nowSelectFile, std::wstring& nowSelectFileName)
{
	...
	if (ImGui::Begin("aaaaa"))
	{
		...
		{
			// Set payload to carry the index of our item (could be anything)
			ImGui::SetDragDropPayload("DND_DEMO_CELL", &buttonId1, sizeof(int));
			if (mode == Mode_Swap) { ImGui::Text("Swap %s", buttonId1); }
			ImGui::EndDragDropSource();
		}
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DND_DEMO_CELL"))
			{
				...
				if (mode == Mode_Swap)
				{
					const char* tmp = names[buttonId1];
					names[buttonId1] = names[payload_n];
					names[payload_n] = tmp;
				}
			}
			ImGui::EndDragDropTarget();
		...
		{
			// Set payload to carry the index of our item (could be anything)
			ImGui::SetDragDropPayload("DND_DEMO_CELL", &buttonId2, sizeof(int));
			if (mode == Mode_Swap) { ImGui::Text("Swap %s", buttonId2); }
			ImGui::EndDragDropSource();
		}
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DND_DEMO_CELL"))
			{
				...
				if (mode == Mode_Swap)
				{
					const char* tmp = names[buttonId2];
					names[buttonId2] = names[payload_n];
					names[payload_n] = tmp;
				}
			}
			ImGui::EndDragDropTarget();
